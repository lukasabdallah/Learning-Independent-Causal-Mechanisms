...
def train_system(epoch, experts, discriminator, optimizers_E, optimizer_D, criterion, data_train, data_labels, args, writer):
    discriminator.train()
    ...
    for step, (batch_train, batch_label) in enumerate(zip(data_train, data_labels)):
        ...
        loss_D_canon = criterion(scores_canon, torch.ones_like(scores_canon))
        ...
        for i, expert in enumerate(experts):
            ...
            if n_expert_samples > 0:
                ...
                exp_samples = exp_outputs[winning_indexes, i].unsqueeze(dim=1)
                # print("exp_samples")
                # print(exp_samples.size())
                D_E_x_transf = discriminator(exp_samples.detach())
                loss_E = criterion(D_E_x_transf, torch.ones_like(D_E_x_transf))
...
if __name__ == '__main__':
    ...
    checkpt_dir = os.path.join(args.outdir, 'InitializedExperts')
    ...
    if args.load_dataset:
        ...
        transf_set, val_set = torch.utils.data.random_split(
            dataset_train, [30000, 30000])
        transf_set.dataset = copy(dataset_train)
    ...
    #args.model_for_initialized_experts= "patient_n_exp_10_bs_32_lri_0.1_lre_0.001_lrd_0.001_ei_100_e_100_oi_adam_oe_adam_oe_adam_1630679719"
    for i, expert in enumerate(experts):
